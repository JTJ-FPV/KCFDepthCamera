...
void FLED::drawFLED(Mat ImgG, const  string& savepath, const string& windowsName)
{
	char arcnum[128];
...
//	Mat Img_T = ImgG;
	...
	for (int i = 0; i < detEllipses.size(); i++)
	{
		...
		temp.size.height = detEllipses[i].size.width;
		temp.size.width = detEllipses[i].size.height;
		// cout << "temp.size.height / temp.size.width" << temp.size.height / temp.size.width << endl;
		// cout << "temp.size.width / temp.size.height" << temp.size.width / temp.size.height << endl;
		temp.center.x = detEllipses[i].center.y;
		temp.center.y = detEllipses[i].center.x;
		temp.angle = -detEllipses[i].angle;
		ellipse(Img_T, temp, cv::Scalar(0, 0, 255), 2);
	...
	if(windowsName.length() > 0)
		imshow(windowsName, Img_T);
	if(savepath.length() > 0)
		imwrite(savepath, Img_T);
}
void FLED::drawFLED(Mat ImgG, Mat& Img_T, const string& savepath, const string& windowsName)
{
	char arcnum[128];
	Img_T = ImgG.clone();
	// cvtColor(Img_T, Img_T, cv::COLOR_GRAY2BGR);
//	Mat Img_T = ImgG;
	...
	for (int i = 0; i < detEllipses.size(); i++)
	{
		...
		// cout << "temp.size.height / temp.size.width" << temp.size.height / temp.size.width << endl;
		// cout << "temp.size.width / temp.size.height" << temp.size.width / temp.size.height << endl;
		float rateOfHeightandWidth = temp.size.height / temp.size.width;
		float rateOfWidthandHeight = temp.size.width / temp.size.height;
		// cout << temp.size.width << "	" << temp.size.height << endl;
		if(((rateOfHeightandWidth <= 1 && rateOfHeightandWidth >= 0.7) || 
			(rateOfWidthandHeight <= 1 && rateOfWidthandHeight >= 0.7)) &&
			temp.size.height * temp.size.width >= 900)
		{
			temp.center.x = detEllipses[i].center.y;
			temp.center.y = detEllipses[i].center.x;
			temp.angle = -detEllipses[i].angle;
...
void FLED::drawFLED(Mat ImgG, Mat& Img_T, bool &sign, const string& savepath, const string& windowsName)
{
	char arcnum[128];
	Img_T = ImgG.clone();
	// cvtColor(Img_T, Img_T, cv::COLOR_GRAY2BGR);
//	Mat Img_T = ImgG;
	...
	for (int i = 0; i < detEllipses.size(); i++)
	{
		...
		float rateOfWidthandHeight = temp.size.width / temp.size.height;
		// cout << temp.size.width << "	" << temp.size.height << endl;
		if(((rateOfHeightandWidth <= 1 && rateOfHeightandWidth >= 0.6) || 
			(rateOfWidthandHeight <= 1 && rateOfWidthandHeight >= 0.6)) &&
			temp.size.height * temp.size.width >= 20)
		{
			temp.center.x = detEllipses[i].center.y;
			temp.center.y = detEllipses[i].center.x;
			temp.angle = -detEllipses[i].angle;
...
void FLED::drawFLED(Mat ImgG, Mat& Img_T, vector<geometry_msgs::Point> &center,bool &sign, const  string& savepath, const string& windowsName)
{
	char arcnum[128];
	Img_T = ImgG.clone();
	// cvtColor(Img_T, Img_T, cv::COLOR_GRAY2BGR);
//	Mat Img_T = ImgG;
	...
	for (int i = 0; i < detEllipses.size(); i++)
	{
		...
		if(((rateOfHeightandWidth <= 1 && rateOfHeightandWidth >= 0.6) || 
			(rateOfWidthandHeight <= 1 && rateOfWidthandHeight >= 0.6)) &&
			temp.size.height * temp.size.width >= 300)
		{
			center[count].x = detEllipses[i].center.x;
			center[count].y = detEllipses[i].center.y;
			center[count].z = 0;
			temp.center.x = detEllipses[i].center.y;
			temp.center.y = detEllipses[i].center.x;
			temp.angle = -detEllipses[i].angle;
