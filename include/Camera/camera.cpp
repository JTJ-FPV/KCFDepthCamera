...
void DepthCamera::init_publisher()
{
    ...
    // 发布机体坐标系下的目标点
    body_level_position = nh.advertise<geometry_msgs::PoseStamped>
        ("/camera/ellipse/body/level/center", 1, this);
...
void DepthCamera::init_subscriber()
{
    ...
    home_position_sub = nh.subscribe<geometry_msgs::PoseStamped>
        ("/CADC/HomePosition", 1,&DepthCamera::homoePositioncb, this);
}
    ...
    try{
        ...
        {   
            ...
            if(okeco)   // 跟踪成功
            {
                ...
                else if(ros::Time::now() - last < ros::Duration(1))
                {
                    // point_aim.pose.position.x
                    // nowRoi_tracker.x,
                    Eigen::Vector2d pixel_center(nowRoi_tracker.x, nowRoi_tracker.y);
                    Eigen::Vector3d p_3d;
                    if(Point3d(depth, pixel_center, p_3d))
                    {
                        point_aim.pose.position.x = p_3d.x();
                        point_aim.pose.position.y = p_3d.y();
                        point_aim.pose.position.z = p_3d.z();
...
void DepthCamera::odomCallback(const nav_msgs::OdometryConstPtr &odom)
{
    ...
    tf::Matrix3x3 R_WB;
    R_WB.setRotation(quat);
    R_WB_ << R_WB[0][0], R_WB[0][1], R_WB[0][2],
...
void DepthCamera::homoePositioncb(const geometry_msgs::PoseStamped::ConstPtr &hp)
{
    home_position_ = *hp;
}
...
{  
    R_Lb_ = Eigen::AngleAxisd(-q(0), Eigen::Vector3d::UnitY()) *
            Eigen::AngleAxisd(q(1), Eigen::Vector3d::UnitX());
    R_Lc_bc_ = Eigen::AngleAxisd(q(0), Eigen::Vector3d::UnitY()) * 
               Eigen::AngleAxisd(q(1), Eigen::Vector3d::UnitX());
    R_WB_ = Eigen::AngleAxisd(-q(2), Eigen::Vector3d::UnitZ()) * R_Lb_;
...
{
    Eigen::Vector3d t(p.pose.position.x - home_position_.pose.position.x, p.pose.position.y - home_position_.pose.position.y, p.pose.position.z - home_position_.pose.position.z);
    ROS_ERROR_STREAM("the pretranslate is " << t.x() << ', ' << t.y() << ',' << t.z());
    T_WB_.rotate(R_WB_);
    T_WB_.pretranslate(t);
}

// void DepthCamera::homoePositionIsometry(const geometry_msgs::PoseStamped::ConstPtr &hp)
// {
//     Eigen::Vector3d t((*hp).pose.position.x, (*hp).pose.position.y, (*hp).pose.position.z);
//     T_HW_.rotate(Eigen::Matrix3d::Identity());
//     T_HW_.pretranslate(t);
// }
    ...
    for(int j = 1; j <= ds.getClusterPoint(); ++j)
    {
        for(int i = 0; i < ds.m_points.size(); ++i)
        {
            ...
            if(sign)
            {
                ...
                if(level_average_depth > 0.1 && level_average_depth <= pose.pose.position.z - 0.36 - 0.05)
                // if(level_average_depth > 0.1 && level_average_depth <= 0.7)
                {
                    ...
                    // ROS_INFO_STREAM("pose.pose.position.z - 0.05 :" << pose.pose.position.z - 0.05 << " " <<i);
                }
            }
        }
    }
...
bool DepthCamera::choosePoint(const std::vector<CADC::classifter> &classifter, DBSCAN::DBSCAN &ds)
{
    ...
    if(Point3d(depth, pixel, aim))
    {
        ...
        // 机体坐标系
        aim = R_b_bc_ * aim + Eigen::Vector3d(body_offset_x_, body_offset_y_, height_);
        Eigen::Vector3d aim_level = R_WB_ * aim;
        // 将机体坐标系转换为水平
        body_level_point_aim.pose.position.x = aim_level(0);
        body_level_point_aim.pose.position.y = aim_level(1);
        body_level_point_aim.pose.position.z = aim_level(2);
        ...
        // ENU坐标系下的坐标
        // aim = T_WB_ * aim;
        Eigen::Vector3d t(odom.pose.pose.position.x - home_position_.pose.position.x, odom.pose.pose.position.y - home_position_.pose.position.y, odom.pose.pose.position.z - home_position_.pose.position.z);
        aim = aim_level + t;
        ...
        // 设置时间戳
        point_aim.header.stamp = ros::Time::now();
        body_level_point_aim.header.stamp = ros::Time::now();
...
bool DepthCamera::classifiter2DpointSet(const std::vector<geometry_msgs::PoseStamped> &aim_point_set, DBSCAN::DBSCAN &ds)
{
    ...
    {
        // ROS_INFO_STREAM("the gloabl point is " << aim_point_set.at(i).pose.position.x << ", "<< aim_point_set.at(i).pose.position.y << ", " << aim_point_set.at(i).pose.position.z);
...
void DepthCamera::calc_cb(const ros::TimerEvent&)
{
    ...
    if(point_aim_.data)
    {
        ...
        if(aim_point_set.size() > 50)
        {
            aim_point_set.clear();
        }
        last_point_aim = point_aim;
        /***************realflight****************/
        body_level_position.publish(body_level_point_aim);
    }
    if(publish_glob_point)
    {
        s.data = true;
    ...
    // Servo Control
    servo_pub.publish(point_aim_);
    // ROS_DEBUG_STREAM("global point : " <<  point_aim.pose.position.x << ", " << point_aim.pose.position.y << ", " << point_aim.pose.position.z);
    ROS_DEBUG_STREAM("body level point : " <<  body_level_point_aim.pose.position.x << ", " << body_level_point_aim.pose.position.y << ", " << body_level_point_aim.pose.position.z);
    ROS_DEBUG_STREAM("global point : " <<  point_aim.pose.position.x << ", " << point_aim.pose.position.y << ", " << point_aim.pose.position.z);
}   
} // namespace CADC